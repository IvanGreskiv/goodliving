...
?>
    ...
    ?>
      ...
      <?php if ( !is_user_logged_in() ) : ?>
				<p>
					<a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=register" class="button"><?php _e('Присоедниться сейчас!', 'colabsthemes') ?></a>
					<span>or</span>
          <a href="<?php echo get_option('siteurl'); ?>/wp-login.php" class="button"><?php _e('Войти', 'colabsthemes') ?></a>
        </p>  
      <?php else: ?>
        <h4 class="widget-title"><span><?php _e('Ваш профиль:', 'colabsthemes');?></span></h4>

        <div class="user-profile-wrapper">
	        <div class="avatar"><?php echo get_avatar($current_user->user_email, 100);?></div>
					<div class="user">
					
	        <p class="welcome-back"><?php _e('Добро пожаловать!,','colabsthemes'); ?> <a href="<?php echo get_author_posts_url($current_user->ID); ?>"><strong><?php echo $display_user_name; ?></strong></a></p>
	        <ul class="user-info">	
						<li><?php _e('Зарегистрирован с:','colabsthemes')?> <strong><?php echo $the_reg_date; ?></strong></li>
      ...
      <h4 class="widget-title"><span><?php _e('Настройки пользователя', 'colabsthemes');?></span></h4>
			<ul class="users-listing">
			<?php if ( is_user_logged_in() ) : 
			$logout_url = wp_logout_url( home_url() );
			?>
				<li><a href="<?php echo CL_DASHBOARD_URL ?>"><?php _e('Моя доска объявлений','colabsthemes')?></a></li>
				<li><a href="<?php echo CL_PROFILE_URL ?>"><?php _e('Редактировать профиль','colabsthemes')?></a></li>
				<?php if (current_user_can('edit_others_posts')) { ?><li><a href="<?php echo get_option('siteurl'); ?>/wp-admin/"><?php _e('Администратор','colabsthemes')?></a></li><?php } ?>
				<li><a href="<?php echo $logout_url; ?>"><?php _e('Выйти','colabsthemes')?></a></li>
				
			<?php else: ?>
				<li><?php _e('Welcome,','colabsthemes'); ?> <strong><?php _e('visitor!','colabsthemes'); ?></strong></li>
							<li><a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=register"><?php _e('Зарегистироваться','colabsthemes'); ?></a></li>
							<li><a href="<?php echo get_option('siteurl'); ?>/wp-login.php"><?php _e('Войти','colabsthemes'); ?></a></li>
			<?php endif; ?>
			</ul>
    </aside>
