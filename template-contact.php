...
if(isset($_POST['submitted'])) {
	...
	if(trim($_POST['checking']) !== '') {
		...
		if(trim($_POST['contactName']) === '') {
			$nameError =  __('Вы забыли ввести свое имя.', 'colabsthemes'); 
			$hasError = true;
		} else {
		...
		if(trim($_POST['email']) === '')  {
			$emailError = __('Вы забыли ввести Email.', 'colabsthemes');
			$hasError = true;
		} else if (!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_POST['email']))) {
			$emailError = __('Ваш Email введен неправильно.', 'colabsthemes');
			$hasError = true;
		} else {
		...
		if(trim($_POST['comments']) === '') {
			$commentError = __('Вы забыли ввести комментарий.', 'colabsthemes');
			$hasError = true;
		} else {
