...
function colabs_login_form( $action = '', $redirect = '' ) {
	...
	<form action="<?php echo $action; ?>" method="post" class="loginform">
		...
		<div id="checksave">
			...
			<p class="lostpass">
				<a class="lostpass" href="<?php echo site_url('wp-login.php?action=lostpassword', 'login') ?>" title="<?php _e('Password Lost and Found', 'colabsthemes'); ?>"><?php _e('Lost your password?', 'colabsthemes'); ?></a><br>
				<?php wp_register('',''); ?>
			</p>
