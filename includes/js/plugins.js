...
/**
 * jQuery Select Options Replacement
 * Turn boring select options into styleable block
 */
(function($){
  $.fn.selectbox = function() {
    return this.each(function(){
      var $el = $(this),
          val = $el.val(),
          textContent = $el.find('option:selected').text(),
          $text;

      // Build Elements
      $el.wrap('<div class="select-wrapper '+ $el.attr('class') +'"></div>');
      $('<span>'+ textContent +'</span><i class="icon-chevron-down"></i>').insertBefore( $el );
      $text = $el.prevAll('span');
...
;(function ($, window, document) {
  ...
  var
    ...
    optionTemplate = '<li class="{{ current }} {{ level }}"><a style="{{ styles }}" data-dk-dropdown-value="{{ value }}">{{ text }}</a></li>',
  ...
  function _build (tpl, view) {
    ...
    if (view.options && view.options.length) {
      for (var i = 0, l = view.options.length; i < l; i++) {
        ...
        oTemplate = oTemplate.replace('{{ text }}', $option.text().replace(/^\s*/g, ''));

        if( typeof $option.attr('class') != 'undefined' ) {
        	var padding_val = $option.attr('class').charAt( $option.attr('class').length - 1 )
        	oTemplate = oTemplate.replace('{{ level }}', $option.attr('class'));
        	oTemplate = oTemplate.replace('{{ styles }}', 'padding-left: ' + ( 13 + padding_val * 8 ) + 'px;' );
        } else {
        	oTemplate = oTemplate.replace('{{ styles }}', '');
        	oTemplate = oTemplate.replace('{{ level }}', '');
        }
...
/**
 * Custom Behaviour for InfiniteScroll
 * 
 */
(function($){
$.extend($.infinitescroll.prototype,{
	...
	_setup_manual_trigger: function infscr_setup_manual_trigger() {
		...
		// Bind loadmoreButton link to retrieve post
		...
		instance.options.loading.start = function (opts) {
		  $(opts.loadmoreButton).fadeTo('fast', 0.2, function(){
	...
	_showdonemsg_manual_trigger: function infscr_showdonemsg_manual_trigger() {
	  ...
	  $(opts.loadmoreButton).fadeTo('fast', 1).text('No More Post').addClass('destroyed');
